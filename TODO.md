...
| F1e | geistlib, Dispatch-Fix (in Arbeit) | ASan instrumentiert den Prolog der AVX-512-TU EVEX-codiert, also vor dem Guard in `kernel_q4kx8_gemm_avx512_full.c:815`. Auf CPUs ohne AVX-512 stirbt das mit SIGILL. Entscheidung des Users: an der Ursache beheben, Guard und Fallback in eine TU ohne `-mavx512*` |
